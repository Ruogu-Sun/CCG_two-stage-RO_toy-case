...
eta = MP.addVar(obj=1, lb=0, name='η') # A continuous variable in the master problem with an objective coefficient of 1.

# Constraints
MP_Cons_1 = MP.addConstrs((z[i] <= 800*y[i] for i in range(3)), name='MP_Cons_1') # For each y[i], it imposes a constraint that z[i] <= 800*y[i].
MP_Cons_2 = MP.addConstr((gp.quicksum(z[i] for i in range(3)) >= 772), name='MP_Cons_2') # Total z[i] values must be at least 772. (Ensures SP2 has feasible region)

# iteration constraints
MP_Cons_3 = MP.addConstrs((gp.quicksum(x[i,j] for j in range(3)) <= z[i] for i in range(3)), name='MP_Cons_3') # For each x[i,j], the sum of x[i,j] values cannot exceed z[i]. Not over inventory
MP_Cons_4 = MP.addConstrs((gp.quicksum(x[i, j] for i in range(3)) >= d[j] for j in range(3)), name='MP_Cons_4') # For each demand variable d[j], the sum of x[i,j] must be greater than or equal to d[j]. Must satisfy demand
MP_Cons_eta = MP.addConstr(eta >= gp.quicksum(x[i,j]*C[i][j] for i in range(3) for j in range(3)), name='MP_Cons_eta') # Constraint on eta, requiring it to be greater than or equal to the weighted sum of x[i,j]*C[i,j]. # eta >= b^T x. See note. This is also the cutting plane


# Master-problem uncertainty constraints
MP_Cons_uncertainty_1 = MP.addConstrs((d[i] == dl[i]+du[i]*g[i] for i in range(3)), name='MP_Uncertainty_Cons1') # Defines d[i] as dl[i] + du[i]*g[i], capturing uncertainty in demand.
MP_Cons_uncertainty_2 = MP.addConstr((gp.quicksum(g[i] for i in range(3)) <= 1.8), name='MP_Uncertainty_Cons2')
MP_Cons_uncertainty_3 = MP.addConstr((gp.quicksum(g[i] for i in range(2)) <= 1.2), name='MP_Uncertainty_Cons3') # Constraints on the sum of g[i] variables. Predefined
...
pi = SP.addVars(6, lb=0, vtype=GRB.CONTINUOUS, name='pi') # The number of Lagragian Multiplier is 6, 3 for long constraints, 3 for x bounds constraints
...
SP_Cons_1 = SP.addConstrs((gp.quicksum(x_sub[i,j] for j in range(3)) <= z[i].x for i in range(3)), name='SP_Cons_1') #[0:3] Not over inventory. z[i].x gives the optimized (current solution) value of ( z[i] ) after solving MP. This is a fixed number in the subproblem (SP), as it is based on the outcome of MP
SP_Cons_2 = SP.addConstrs((gp.quicksum(x_sub[i,j] for i in range(3)) >= d_sub[j] for j in range(3)), name='SP_Cons_2') #[3:6] Meet all demands
SP_Cons_3 = SP.addConstrs((-pi[i]+pi[j+3] <= C[i][j] for i in range(3) for j in range(3)), name='SP_Cons_3') #[6:15] SP_Cons_3 deals with pricing constraints based on cost matrix C. \theta_j - \pi_i <= c_{ij}
...
while np.abs(UB-LB)>1e-5 :
    ...
    MP_Cons_3_new = MP.addConstrs((gp.quicksum(x_new[i,j] for j in range(3)) <= z[i] for i in range(3)), name='MP_Cons_3_{}'.format(k)) # new "no more than inventory"
    MP_Cons_4_new = MP.addConstrs((gp.quicksum(x_new[i, j] for i in range(3)) >= d_sub[j].x for j in range(3)), name='MP_Cons_4_{}'.format(k)) # new "demand must be satisfied"
    MP_Cons_eta = MP.addConstr(eta >= gp.quicksum(x_new[i,j]*C[i][j] for i in range(3) for j in range(3)), name='MP_Cons_eta_{}'.format(k)) # new "cutting plane added"
    MP.optimize()
    LB = max(LB, MP.objval)
    print(LB)
